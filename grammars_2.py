...
class Production:
    def __init__(self, left, length, right, minSpan, maxSpan, tws, twl, cost):
        self.left = int(left)
...
class AndNode:
    def __init__(self, leftChild, rightChild, parent, production, idN):
        ...
        self.decisionVariable = None
        self.cost = self.production.cost
...
def readGrammar(data):
    ...
    nbNonTerminals = 5 + len(data.shifts) * 4
    # S-> len(data.shifts) + 1
    # R-> len(data.shifts) + 2
    # F-> len(data.shifts) + 3
    # Q-> len(data.shifts) + 4
    # M-> len(data.shifts) + 5
    # A_s-> len(data.shifts) + 6 + i (len(data.shifts))
    # A_n_s
    # A_s_2
    # A_s_3
    ...
    ps_s1 = Production(len(data.shifts) + 1, 2, [len(data.shifts) + 2, len(data.shifts) + 3],
                       0, periods_grammar, 0, periods_grammar, 0.0)
    ...
    # S->FM self, left, length, right, minSpan, maxSpan, tws, twl, cost
    ps_s4 = Production(len(data.shifts) + 1, 2, [len(data.shifts) + 3, len(data.shifts) + 5],
                       0, periods_grammar, 0, periods_grammar, 0.0)
    g.addProduction(ps_s4)
    ...
    #                                            data.shifts[i].tws, data.shifts[i].twl, 0.0)
    #     g.addProduction(ps)


    #check later
    for i in range(len(data.shifts)):
        prod = Production(len(data.shifts) + 3, 2, [len(data.shifts) + 6 + i , len(data.shifts) * 2 + 6 + i],
                           2, 6, 0, periods_grammar, 0.0)
        g.addProduction(prod)

        prod = Production(len(data.shifts) + 3, 2, [len(data.shifts) *3 + 6 + i, len(data.shifts) * 4 + 6 + i],
                          2, 6, 0, periods_grammar, 0.0)
        g.addProduction(prod)
        ...
        for j in range(len(data.shifts)):
            if i != j:
                ...
                transition_cost = 0
                if(data.shifts[i].endPeriod - data.shifts[i].startPeriod < 12):
                    transition_cost = 1000

                prod = Production(len(data.shifts) * 2 + 6 + j, 2, [len(data.shifts)  + 6 + i, len(data.shifts) * 2 + 6 + i],
                                  1, periods_grammar, 0, periods_grammar, transition_cost)
                g.addProduction(prod)

                prod = Production(len(data.shifts) * 2 + 6 + j, 2,
                                  [len(data.shifts) *3 + 6 + i, len(data.shifts) * 4 + 6 + i],
                                  1, periods_grammar, 0, periods_grammar, 0.0)
                g.addProduction(prod)

        prod = Production(len(data.shifts)  + 6 + i, 2, [len(data.shifts) *3 + 6 + i, len(data.shifts) * 4 + 6 + i],
                          2, 6, 0, periods_grammar, 0.0)
        g.addProduction(prod)

        prod = Production(len(data.shifts) * 3 + 6 + i, 2, [len(data.shifts) * 3 + 6 + i, len(data.shifts) * 4 + 6 + i],
                          1, periods_grammar, 0, periods_grammar, 0.0)
        g.addProduction(prod)

        prod = Production(len(data.shifts) * 3 + 6 + i, 1, [i],
                          1, periods_grammar, 0, periods_grammar, 0.0)
        g.addProduction(prod)

        prod = Production(len(data.shifts) * 4 + 6 + i, 1, [i],
                      1, periods_grammar, 0, periods_grammar, 0.0)
        g.addProduction(prod)


    # R->RR self, left, length, right, minSpan, maxSpan, tws, twl, cost
    ps_RR = Production(len(data.shifts) + 2, 2, [len(data.shifts) + 2, len(data.shifts) + 2],
                      1, 2, 0, periods_grammar, 0.0)
    g.addProduction(ps_RR)

    # R->r self, left, length, right, minSpan, maxSpan, tws, twl, cost
    ps_R = Production(len(data.shifts) + 2, 1, [len(data.shifts)],
                       0, 1, 0, periods_grammar, 0.0)
    ...
    # M->RF self, left, length, right, minSpan, maxSpan, tws, twl, cost
    ps_Q = Production(len(data.shifts) + 5, 2, [len(data.shifts) + 2, len(data.shifts) + 3],
                      0, periods_grammar, 0, periods_grammar, 0.0)
    g.addProduction(ps_Q)

    # # R->ZR self, left, length, right, minSpan, maxSpan, tws, twl, cost
    # ps_ZR = Production(len(data.shifts) + 2, 2, [len(data.shifts) + 4, len(data.shifts) + 2],
    #                   1, 2, 0, periods_grammar)
    # g.addProduction(ps_ZR)
    #
    # # R->r self, left, length, right, minSpan, maxSpan, tws, twl, cost
    # ps_R = Production(len(data.shifts) + 2, 1, [len(data.shifts)],
    ...
    # g.addProduction(ps_R)
    #
    # # Z->r self, left, length, right, minSpan, maxSpan, tws, twl, cost
    # ps_Z = Production(len(data.shifts) + 4, 1, [len(data.shifts)],
    #                   0, 1, 0, periods_grammar)
    # g.addProduction(ps_Z)
    ...
    for node in graph.postOrderNodes:
        for n in node.children:
            if n.decisionVariable == None:
                ...
                if n.cost> 0:
                    m.production_cost += n.decisionVariable * n.cost
                    #print ("cost ", n.cost)
