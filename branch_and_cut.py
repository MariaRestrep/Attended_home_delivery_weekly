...
def define_master(param, grammar_graph, grammar):
    ...
    model.minimize(model.fixed_cost + model.theta_cost + model.production_cost)
...
def solve_bac(param, grammar_graph, grammar, log_output=False, timeout=1800, obj_tolerance=1e-01, abs_obj_tolerance=1e-01, epsilon_tolerance=0.05,
    ...
    try:
        ...
        # print("*************************** Solution ***************************")
        # print(param.C, param.D, len(param.data.shifts))

        x_sol = [0] * len(param.C)
        for c in range(len(param.C)):
            x_sol[c] = [0] * len(param.D)
            for d in range(len(param.D)):
                x_sol[c][d] = [0] * (len(param.data.shifts))

        # print("init x")

        cost_shifts = 0
        lenght_tour = [0]*len(param.C)
        ...
        for c in param.C:
            for d in param.D:
                for s in range(len(param.data.shifts)):
                    if problem.x[c,d,s].solution_value > 0.01:
                        # print(c, d, s, problem.x[c,d,s].solution_value)
                        lenght_tour[c] += param.data.shifts[s].workTime
                        cost_shifts += param.data.shifts[s].cost
            # print(c, lenght_tour[c])
        ...
        #             print(c, d, s, var.solution_value, lenght_tour[c])
        #             lenght_tour[c] += param.data.shifts[s].workTime
        #             cost_shifts += param.data.shifts[s].cost
        ...
        for c in param.C:
            # print(c, lenght_tour[c])
            average_tour_l += lenght_tour[c]

            if (lenght_tour[c] < min_tour_l):
                min_tour_l = lenght_tour[c]

            if (lenght_tour[c] > max_tour_l):
                max_tour_l = lenght_tour[c]
        ...
        problem.cost_shifts =  cost_shifts
